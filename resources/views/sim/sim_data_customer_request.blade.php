                                    ...
                                    <table class="table table-hover my-0" id="dataTable" style="border-collapse: collapse; width: 100%">
                                        ...
                                        <tbody>
                                            ...
                                            <tr data-id="{{ $item->id }}">
                                                ...
                                                <td>{{ $item->simData->admin->username ?? 'N/A' }}</td>
                                                <td>{{ $item->simData->phone_number ?? 'N/A' }}</td>
    ...
    document.querySelectorAll('.confirm-btn').forEach(button => {
        button.addEventListener('click', function() {
            ...
            if (!this.disabled && confirm('Bạn có chắc chắn muốn xác nhận?')) {
                fetch(`/confirm-item-simdata/${itemId}`, {
                        method: 'POST',
                        headers: {
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            row.querySelector('td:nth-child(8)').innerText = 'Đã xác nhận'; // Cập nhật trạng thái
                            this.disabled = true;
    ...
    document.querySelectorAll('.cancel-btn').forEach(button => {
        button.addEventListener('click', function() {
            ...
            if (confirm('Bạn có chắc chắn muốn hủy yêu cầu này?')) {
                fetch(`/cancel-item-simdata/${itemId}`, {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
                    ...
                    .then(data => {
                        if (data.success) {
                            let row = this.closest('tr');
                            row.querySelector('td:nth-child(8)').innerText = 'Đã huỷ bỏ'; // Cập nhật trạng thái
                            this.disabled = true;
